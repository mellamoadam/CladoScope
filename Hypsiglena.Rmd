...
```{r USAGE INSTRUCTIONS, echo = FALSE} 


# A limitation of this code is that there is some circular dependencies. First, we use matchDF in the CREATE POPULATION MAP section based on our best guess for a population map. Then in the section VCF FILTERING, we filter each subset separately, but some of these population assignments might not be accurate. This allows us to define the best filter sets to use in DAPC, which gives us group assignments for each sample in their respective subset. Once that is done, we can rerun the CREATE POPULATION MAP section with a more accurate matchDF that we manually update, and then run VCF FILTERING, and DAPC.

# This is necessary because running DAPC on an entire dataset might not give meaningful results if there is a large amount of diversity. For example, in the Hypsiglena dataset, concatenated trees clearly separate H. unaocularus from H. chlorophaea, but at the same K value, it receives the same group assignment as H. chlorophaea, while H. jani (Central Clade) is split in a way that doesn't agree with other analyses like concatenated trees or PCA. 

# Simply assuming the subset a sample belongs to based on geography or morphological characteristics is antithetical to the whole reason for doing genetic analyses, however, it can give us a good starting point (just to determine decent filtering schemes and initial DAPC assignments). For example, in the Hypsiglena dataset, we have an idea of population assignments based on previous studies, geographical barriers, and morphology. One subset we create is all of H. jani, which we suspect may have distinct populations nested within since it spans a large range of varying habitats and varying phenotpyes. Another subset is the H. chlorophaea group. We see one sample that is in the four-corners area that doesn't seem to fit in with H. chlorophaea based on concatenated trees. Also the DAPC group assignment is with the much further south group, while we might expect it to cluster better with H. chlorophaea loreala. Since the sample better fits with H. jani, we update matchDF and rerun all analyses.
...
if (!file.exists(inatCoordsFile)) {
  inatData = get_inat_obs(query="Hypsiglena", maxresults = 10000)
...
```{r MAP RELATED ORGANIZATION, echo = FALSE}
...
```{r SET MAP PARAMETERS, echo = FALSE}
...
# Must uncomment to run filtering sets. Takes a long time otherwise, and only need to run once
```{r VCF FILTERING, echo = TRUE}
...
```{r BEST FILTER SET VCF FILE CONVERSIONS, echo = FALSE}
...
```{r GROUP ASSIGNMENT DF CREATION, echo = FALSE}
...
```{r Color Coordinated Plots, echo = FALSE} 

################################# USER INPUTS  #################################
# Grab the tree with missingSitesThresholdBestConcatenatedTree value
bestTreeFile = list.files(path = paste0(folderPath, PHYPath), pattern = paste0("MissingSites", missingSitesThresholdBestConcatenatedTree, ".+treefile"))

outgroupColor = hybridColor = "#000000"

# Only use the following subsets for admixture and PCA admixture
colorCoordinationSubsetsOfInterest = grep("ochrorhyncha|ChlorophaeaSpNov1SpNov2TorquataUnaocularusCatalinae|jani", AdmixtureBestFilterList, value=TRUE, ignore.case = TRUE)

################################################################################

######### DAPC
colorCoordinationPDFPath = paste0(folderPath, pdfPath, "colorCoordinatedPDFList.pdf")
pdf(colorCoordinationPDFPath)

# For bounds on map plot
latitudeLowerLimit = min(grpDFPopmapCoordsDFAllColorCoordination$Latitude,na.rm = TRUE) - mapBoundsBuffer
latitudeUpperLimit = max(grpDFPopmapCoordsDFAllColorCoordination$Latitude,na.rm = TRUE) + mapBoundsBuffer
longitudeLowerLimit = min(grpDFPopmapCoordsDFAllColorCoordination$Longitude,na.rm = TRUE) - mapBoundsBuffer
longitudeUpperLimit = max(grpDFPopmapCoordsDFAllColorCoordination$Longitude,na.rm = TRUE) + mapBoundsBuffer
inatCoords
...
# Add inat coords
geom_point(data = inatCoords, aes(x = longitude, y = latitude), color = "grey50", alpha = 0.5, size = 0.8) +

geom_point(data = grpDFPopmapCoordsDFAllColorCoordination, aes(x = Longitude, y = Latitude, fill = Color), size = 4, alpha= 0.7, shape = 21, color = "black", stroke = 0.5) +
geom_text(data = grpDFPopmapCoordsDFAllColorCoordination, aes(x = Longitude, y = Latitude, label = UniqueNumber), color = "black", size = 2, fontface = "bold", vjust = 0.5, hjust = 0.5) +
labs(title = "PCA-LDA Group Assignments with Number of Clusters\n Chosen by K-means Minimum AIC", x = "", y = "") +
theme_minimal() +
theme(plot.title = element_text(hjust = 0.5)) +
theme(legend.title = element_blank(), legend.position = "none") + 
...
currentTreeFile = bestTreeFile
currentTreeFileString = sub("\\.phy\\.treefile$", "", currentTreeFile)
...
currentSampleList = system(paste0("awk 'BEGIN{FS=\"\t\"} /^#CHROM/ {for (i=10; i<=NF; i++) print $i}' ", folderPath, VCFPath, currentTreeFileString, ".vcf"), intern = TRUE)
...
if (length(currentTreeFile) != 0) {
  #Create popmap for tree tip labels for color coordination to population names
  treeMatchDF = data.frame(Sample=tree$tip.label) #initialize popmap with IDs
  treeMatchDF$Population = TreeColPopMap$Population[match(treeMatchDF$Sample, TreeColPopMap$Sample)]
  treeMatchDF$Color = TreeColPopMap$Color[match(treeMatchDF$Sample, TreeColPopMap$Sample)]
  
  treeMatchDF[treeMatchDF$Sample %in% outgroupSamples,]$Population = outgroupPopulation
  treeMatchDF[treeMatchDF$Sample %in% outgroupSamples,]$Color = outgroupColor
  
  
  titleSplit = paste(substr(currentTreeFileString, 1, nchar(currentTreeFileString) %/% 2), substr(currentTreeFileString, nchar(currentTreeFileString) %/% 2 + 1, nchar(currentTreeFileString)), sep="\n")

  plot.phylo(ladderize(tree),main=titleSplit,cex=0.2,cex.main=0.6,show.node.label = TRUE, tip.color=treeMatchDF$Color) 
ape::write.tree(tree, file= paste0(folderPath, pdfPath, "ColorCoordinatedIQTree.tre"))
metadataFile = paste0(folderPath, pdfPath, "ColorMetadataForFigTree.tsv")

write.table(treeMatchDF[,c("Sample","Color")], 
            file=metadataFile, 
            sep="\t", 
            row.names=FALSE, 
            quote=FALSE)
...
# Admixture and admixture PCA

for (bestFilterBySubset in colorCoordinationSubsetsOfInterest) {


# Grab the name of the subset populations based on it's position in the file string (between rawString and "Outgroup")
currentSubsetName = sub(paste0("^", rawString, "(.*?)(Outgroup).*"), "\\1", bestFilterBySubset)

# Subset the df containing colors, samples, populations etc to only include samples in this admixture subset
admixColorCoordinationDF = grpDFPopmapCoordsDFAllColorCoordination[grpDFPopmapCoordsDFAllColorCoordination$Subset == currentSubsetName,]

# Remove any samples if they weren't included in the analysis
admixColorCoordinationDF = admixColorCoordinationDF[!(admixColorCoordinationDF$Sample %in% samplesToRemoveAdmixture),]
rownames(admixColorCoordinationDF) = NULL


# Find the number of populations in the current subset
currentSubsetPopulationLength = length(unique(admixColorCoordinationDF$Color))


K = currentSubsetPopulationLength
...
# In some cases, samples may have been removed from DAPC analysis that were present in admixture, so delete those samples
indexNotInAdmixture = which(!(famDF$Sample %in% admixColorCoordinationDF$Sample))

if (length(indexNotInAdmixture) > 0) {  
  famDF = famDF[-indexNotInAdmixture,]  
  Q = Q[-indexNotInAdmixture,]
}

# Match the samples to the population
QJoined = inner_join(famDF, admixColorCoordinationDF, by = "Sample") 


QJoined = cbind(QJoined[c("Sample", "Color", "Latitude", "Longitude")], Q)
QJoined = QJoined[order(QJoined$Color),]

Q_long = melt(QJoined, id.vars = c("Sample", "Color","Latitude", "Longitude"), variable.name = "AncestryComponent", value.name = "Proportion")


# Bar plot
# Compute average proportion of each ancestry component for each unique color
colorToAncestry=aggregate(QJoined[,5:(ncol(QJoined))],by=list(QJoined$Color),FUN=mean)

# Identify the dominant ancestry component for each color
dominantAncestry=apply(colorToAncestry[,-1],1,function(x)names(which.max(x)))

# Create a mapping of ancestry components to colors
ancestryColorMapping=setNames(colorToAncestry$Group.1,dominantAncestry)

# Apply the mapping to the ggplot fill scale
p=ggplot(Q_long,aes(x=Sample,y=Proportion,fill=AncestryComponent))+
  geom_bar(stat="identity")+
  labs(x=" ",y="Ancestry",fill="Ancestry Component")+
  scale_fill_manual(values=ancestryColorMapping)+
  theme_minimal()+
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
# For bounds on map plot
latitudeLowerLimit = min(QJoined$Latitude,na.rm = TRUE) - mapBoundsBuffer
latitudeUpperLimit = max(QJoined$Latitude,na.rm = TRUE) + mapBoundsBuffer
longitudeLowerLimit = min(QJoined$Longitude,na.rm = TRUE) - mapBoundsBuffer
longitudeUpperLimit = max(QJoined$Longitude,na.rm = TRUE) + mapBoundsBuffer


QMapAdmixtureColnames = colnames(QJoined)[grepl("^V\\d+$", colnames(QJoined))]



# Apply the same ancestry color mapping from the previous plot
p65 = ggplot()+
  # Add in terrain background
  geom_raster(data=terrainDF,aes(x=Longitude,y=Latitude,fill=Color))+
  scale_fill_identity()+ 
  ...
  # Use the same ancestry color mapping as the bar plot
  scale_fill_manual(values=ancestryColorMapping)+  
  theme_void()+  
  theme(legend.position="none")+
  labs(caption=paste(substr(bestFilterBySubset,1,nchar(bestFilterBySubset)%/%2),
                     substr(bestFilterBySubset,nchar(bestFilterBySubset)%/%2+1,nchar(bestFilterBySubset)),
                     sep="\n"))+
  theme(plot.caption=element_text(size=6,hjust=0))+
  coord_sf(xlim=c(longitudeLowerLimit,longitudeUpperLimit), 
           ylim=c(latitudeLowerLimit,latitudeUpperLimit), 
...
# Pull in Traw file that has 0,1,2,NA representing distance from reference allele
currentTrawDF = data.frame(read.table(paste0(folderPath, TrawPath, bestFilterBySubset,".traw") , header=TRUE))[-c(1:6)]
...
# Merge PCA coordinates with ancestry proportions from QJoined
PCA_Plot_Data = merge(currentPCA$li[,1:2],QJoined,by.x="row.names",by.y="Sample")
colnames(PCA_Plot_Data)[2:3]=c("PC1","PC2")

# Range of PC axes seems to affect pie chart sizes, so normalize
radSize = 0.01 * (range(PCA_Plot_Data$PC1)[2]-range(PCA_Plot_Data$PC1)[1] + (range(PCA_Plot_Data$PC2)[2]-range(PCA_Plot_Data$PC2)[1]))
# Plot PCA with admixture pie charts
p121 = ggplot()+
  geom_scatterpie(aes(x = PC1, y = PC2, r = radSize),  
                  data = PCA_Plot_Data,
                  cols = QMapAdmixtureColnames,
                  color="black", alpha = 0.8, linewidth=0.1) +
  coord_fixed() +
  # Ensure color consistency with ancestry components
  scale_fill_manual(values = ancestryColorMapping) + 
  theme_minimal()+
  theme(
    ...
    axis.text=element_text(size = 10),  # Keep axis labels
    axis.title=element_text(size = 12),
    legend.position = "none"
  ) +
  labs(x="PC 1",y="PC 2",title = "PCA with Ancestral Components")

grid.arrange(p121, ncol=1)
...
imgList = image_read_pdf(colorCoordinationPDFPath, density = 1000)  

# Save each page individually
for (i in seq_along(imgList)) {
  image_write(imgList[i], path = paste0(folderPath, pdfPath, "colorCoordinatedPage", i, ".jpg"), format = "jpeg")
}
